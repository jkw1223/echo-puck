...
bool MicroWakeWordEngine::processFeatureFrame(const int8_t* features) {
    ...
    LOGI("MicroWakeWord", "probability=%u", static_cast<unsigned>(probability));
...
bool MicroWakeWordEngine::checkDetection() const {
    ...
    bool detected = sum > static_cast<uint32_t>(probabilityCutoff_) * static_cast<uint32_t>(slidingWindowSize_);
    if (detected) LOGI("MicroWakeWord", "DETECTED sum=%u cutoff=%u window=%u", sum, probabilityCutoff_, slidingWindowSize_);
    return detected;
}
